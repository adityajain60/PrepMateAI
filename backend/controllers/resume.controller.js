...
export const analyzeResume = async (req, res) => {
  try {
    ...
    // 2. Prepare and call Python service
    const { pythonServiceData, headers } = prepareApiRequest(req);
    const analysisResult = await callPythonService('/resume/analyze', pythonServiceData, headers);
    ...
    if (matchScore === undefined || !analysisSummary) {
      return res.status(500).json({ error: 'Incomplete analysis result from AI service' });
    }

    // 3. Save result to history
    const { jobDescription } = req.body;
    const jobDescriptionForDB = jobDescription || `Job from file: ${req.files?.jobDescriptionFile?.[0]?.originalname}`;
