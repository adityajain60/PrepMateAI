...
export const prepareApiRequest = (req) => {
  ...
  if (hasFiles) {
    ...
    // Append files
    if (req.files.resume) {
      formData.append('resume', req.files.resume[0].buffer, { filename: req.files.resume[0].originalname });
    }
    if (req.files.jobDescriptionFile) {
      formData.append('jobDescriptionFile', req.files.jobDescriptionFile[0].buffer, { filename: req.files.jobDescriptionFile[0].originalname });
    }
...
/** 
 * Calls a specified endpoint on the Python service.
 * @param {string} endpoint The endpoint path (e.g., '/resume/analyze').
 ...
 * @returns {Promise<any>} The data from the Python service response.
 */
export const callPythonService = async (endpoint, data, headers, timeout = 60000) => {
